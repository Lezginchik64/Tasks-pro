...
# Тип данных timedelta представляет собой временной интервал (разницу между двумя объектами datetime или date)
# Используется для удобного выполнения различных манипуляций над типами datetime или date.
...
# Аргументы могут быть целыми числами или числами с плавающей запятой, а также могут быть как положительными, так и отрицательными.
...
# Тип timedelta внутренне хранит только сочетание days, seconds, microseconds, а остальные переданные в конструктор аргументы конвертируются в эти единицы:
    # milliseconds преобразуются в 1000 microseconds
    # minutes преобразуются в 60 seconds
    # hours преобразуются в 3600 seconds
    # weeks преобразуются в 7 days
...
# Обратите внимание на то, что у типа timedelta нет атрибутов hours и minutes, позволяющих получить количество часов и минут соответственно.
...
# Для всех других операторов сравнения, таких как <, >, <=, >=, при сравнении объекта timedelta с объектом другого типа возникает ошибка (исключение) TypeError.
from datetime import timedelta

delta1 = timedelta(seconds=57)
delta2 = timedelta(hours=25, seconds=2)

print(delta2 > delta1)     # тут все ок, True
# print(delta2 > 5)
print()
...
# Рассмотрим следующую задачу: рабочая смена длится 7 часов 30 минут. Сколько полных смен в 3 сутках?
all_time = timedelta(days=3)
smena = timedelta(hours=7, minutes=30)
print(all_time // smena)    # 9
print(all_time % smena)     # 4:30:00
print()
# Таким образом, в 3 сутках помещается 9 полных смен и еще остается 4 часа 30 минут.
...
# 2021-01-03
# 2020-12-30
# При прибавлении временного интервала к дате (тип date) неполные сутки отбрасываются.

# Объект типа timedelta также возникает при вычитании двух дат (тип date) или двух значений даты-времени (тип datetime).
from datetime import datetime, date, timedelta
...
# Примечание 2. При работе с типом timedelta мы можем использовать встроенную функцию abs().
