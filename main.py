...
# == Формат csv ==
# CSV (от англ. Comma-Separated Values — значения, разделенные запятыми) — текстовый формат, предназначенный для представления табличных данных.
...
# Рассмотрим текстовый файл products.csv, содержащий информацию о товарах некоторого интернет-магазина.
# Файл содержит информацию о трех столбцах:
...
# Также мы можем отсортировать товары по цене и напечатать 5 самых дешевых товаров.
with open('products.csv', encoding='utf-8') as file:
    data = file.read()
    table = [r.split(',') for r in data.splitlines()]
...
# Также стоит заметить, что в лямбда-функции мы преобразуем элемент item[1] к числовому типу, иначе сортировка будет работать не так, как полагается.

# Столбец keywords может содержать (и, как правило, на практике содержит) несколько ключевых слов.
# В этом случае структура файла нарушается. Для решения такой проблемы можно использовать два подхода.

# Первый подход. Если поле содержит запятые, то это поле должно быть заключено в двойные кавычки.
...
# Обратите внимание на записи с номерами 1, 5, 8, 15 и 18.
# В этих записях в столбце keywords используется символ запятой, и для правильной обработки данных нам необходимо соответствующее поле обрамить двойными кавычками.
...
# Второй подход. Можно использовать в качестве разделителя другой символ, например, символ табуляции (\t), который весьма редко встречается в качестве валидного содержимого файла.
# keywords	price	product_name
# Садовый стул, стул для дачи	1699	ВЭДДО
# Садовый стул	2999	ЭПЛАРО
# Садовый табурет	1699	ЭПЛАРО
...
# tsv — текстовый формат для представления табличных данных. Каждая запись в таблице — строка текстового файла.
# Каждое поле записи отделяется от других символом табуляции, а точнее — горизонтальной табуляции.
...
with open('products.tsv', encoding='utf-8') as file:
    ...
    for line in table[:5]:
        print(line)
# выводит 5 самых дешевых товаров:
    # ['Кружка, сосуд, стакан с ручкой', '39', 'СТЕЛЬНА']
    # ['Ситечко', '59', 'ИДЕАЛИСК']
    # ['Ситечко', '199', 'САККУННИГ']
    # ['Кружка', '199', 'ФИНСТИЛТ']
...
# В каждой итерации цикла при этом будет доступна соответствующая строка файла, уже разбитая запятыми и представляющая собой список.
...
with open('products.csv', encoding='utf-8') as file:
    rows = csv.reader(file, delimiter=';', quotechar='"')   # (в качестве разделителя выбран символ ';')
...
# выводит первые 6 строк файла, включая заголовок с названиями столбцов:
    ...
    # ['Садовый стол', '1999', 'ТЭРНО']
    # ['Складной стол, обеденный стол', '7499', 'ЭПЛАРО']
# При создании reader-объекта мы указываем, что символ-разделитель записей delimiter в нашем файле — точка с запятой, а символ кавычек quotechar — двойные кавычки.
# Кроме того, мы используем встроенную функцию enumerate() для нумерации строк.


# -- Чтение данных с помощью DictReader --
# Иcпользовать reader-объект не всегда удобно, так как он возвращает сырые списки из строк файла, к тому же первой строкой является строка с названиями столбцов, которая практически всегда удаляется, поскольку она мешает правильной обработке данных.
...
# Пусть содержимое файла products.csv имеет следующий вид (в качестве разделителя выбран символ ';'):
    # keywords;price;product_name
    # "Садовый стул, стул для дачи";1699;ВЭДДО
    # Садовый стул;2999;ЭПЛАРО
    # Садовый табурет;1699;ЭПЛАРО
...
# Обратите внимание на то, что переменная row имеет тип dict в Python 3.8 и позднее. В более ранних версиях переменная row имела тип OrderedDict, который мы изучим чуть позже.
import csv
...
# выводит 5 самых дорогих товаров:
    ...
    # {'keywords': 'Садовый стул, стул для дачи', 'price': '1699', 'product_name': 'ВЭДДО'}
    # {'keywords': 'Садовый табурет', 'price': '1699', 'product_name': 'ЭПЛАРО'}
# При создании объекта DictReader значениями по умолчанию для аргументов delimiter и quotechar являются ',' (символ запятой) и '"' (символ двойной кавычки) соответственно.
# Обратите внимание на то, что при использовании DictReader мы не избавляемся от первой строки, содержащей названия столбцов.
# Они хранятся в атрибуте fieldnames объекта DictReader.
# При этом к элементам строк мы обращаемся теперь не по индексам (int(item[1])), а по их названиям (int(item['price'])), что намного удобнее.
...
with open('students.csv', 'w', encoding='utf-8', newline='') as file:
    ...
    for row in data:                         # запись строк
        writer.writerow(row)
# создает файл students.csv со следующим содержимым:
...
# Он отвечает за переводы строк при чтении или записи в текстовый файл и по умолчанию имеет значение None — в этом случае все разделители строк преобразуются в '\n'.
...
# При создании writer-объекта мы также можем его настраивать, задавая delimiter и многие другие параметры.
import csv
...
# По умолчанию символом кавычки является ".
# Если нужно поменять символ, то используйте уже знакомый нам именованный аргумент quotechar.
...
# Мы также можем использовать метод writerows() объекта DictWriter для записи сразу нескольких строк.
# Таким образом, вместо следующих строк кода:
for row in data:
    writer.writerow(row)
# можно написать:
writer.writerows(data)
...
# Примечание 4. Достаточно распространенная ошибка при чтении данных из csv-файла — это передача в csv.reader (csv.DictReader) не файлового объекта, а текстовых данных, считанных из файла.
# Это не вызывает ошибки, но результат будет отличаться от ожидаемого.
#
...
#
# Примечание 6. Так как reader-объект является итератором, мы можем преобразовать его в список или кортеж с помощью встроенных функций list() и tuple() соответственно,
# но не можем преобразовать в множество, так как его элементами могут быть только неизменяемые объекты, которыми списки не являются.